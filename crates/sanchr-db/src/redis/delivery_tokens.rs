...
pub async fn issue_tokens(redis: &Client, count: u32) -> Result<Vec<Vec<u8>>, Error> {
    ...
    // fred 9.x maps Vec<Vec<u8>> → Value::Array which is rejected by
    // Redis SADD. Each member must be Value::Bytes so the client sends
    // a flat bulk-string list, not a nested array.
    let members: Vec<Value> = tokens
        .iter()
        .map(|t| Value::Bytes(Bytes::copy_from_slice(t)))
        .collect();
    redis.sadd::<(), _, _>(DELIVERY_TOKENS_KEY, members).await?;

    // Refresh TTL on every issue call so the set doesn't expire mid-batch.
    redis
        .expire::<(), _>(DELIVERY_TOKENS_KEY, TOKEN_TTL_SECS, None)
        .await?;
...
pub async fn validate_and_consume(redis: &Client, token: &[u8]) -> Result<bool, Error> {
    let removed: i64 = redis
        .srem(
            DELIVERY_TOKENS_KEY,
            Value::Bytes(Bytes::copy_from_slice(token)),
        )
        .await?;
